...
class EntResultadosController extends Controller
{
    ...
    public function setByRegrasFormatacaoAction()
    {
        ...
        $sql = "SELECT t_resultados.ft_observacao , t_resultados.fn_id_modeloresultado ,t_resultados.fd_criacao ,t_resultados.fd_conclusao , t_unidadesmedida.ft_descricao AS medida, t_parametros.ft_descricao , t_resultados.ft_id_estado, t_resultados.fn_id_amostra, t_resultados.fn_calculado,t_resultados.ft_original,t_resultados.ft_prefixo, t_resultados.ft_formatado FROM t_resultados INNER JOIN t_parametros ON t_resultados.fn_id_parametro = t_parametros.fn_id INNER JOIN t_unidadesmedida ON t_unidadesmedida.fn_id = t_resultados.fn_id_unidade  WHERE t_resultados.fn_id_amostra =". $arr[0]['columns'][1] ." AND (t_resultados.ft_id_estado = 'D' OR t_resultados.ft_id_estado = 'C') AND t_parametros.ft_descricao = '". $arr[0]['columns'][2] ."' ";
        $activeDate = $this->getDoctrine()->getManager()->getConnection()->prepare($sql);
        $activeDate->execute();
        $result = $activeDate->fetchAll();
